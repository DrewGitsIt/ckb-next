...
// SABRE RGB PRO Champion Series - a Bragi mouse, unrelated to the legacy NXP SABREs above
#define P_SABRE_RGB_PRO      0x1b79
...
#define SW_PKT_HAS_NO_WHEEL(kb)                     ((kb)->vendor == V_CORSAIR && ((kb)->product == P_SABRE_RGB_PRO || (kb)->product == P_M55_RGB_PRO || (kb)->product == P_KATAR_PRO_XT || (kb)->product == P_KATAR_PRO || (kb)->product == P_SCIMITAR_ELITE_BRAGI))

// For devices that have the new short HID report (6 bytes)
#define USES_BRAGI_SHORT_REPORT(kb)                 ((kb)->vendor == (V_CORSAIR) && ((kb)->product == P_SCIMITAR_ELITE_BRAGI || (kb)->product == P_SABRE_RGB_PRO))
